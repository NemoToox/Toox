...
class Spammer:
    ...
    PHONE_PATTERN = re.compile(r'^20\d{20}$')  # Regex for validating Iranian phone numbers
    ...
    def get_phone_number(cls) -> None:
        ...
        while True:
            phone = input(ConsoleUI.print_styled("info", "Enter the phone number (2XXXXXXXXX): "))
            
            if cls.validate_phone_number(phone):
                cls._phone_number = phone
                break
            else:
                print(Message.show_error("Invalid phone number! Format: 2XXXXXXXXX"))
